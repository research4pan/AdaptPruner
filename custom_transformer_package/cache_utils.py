...
class HybridCache(Cache):
    ...
    def __init__(self, config: PretrainedConfig, max_batch_size, max_cache_len, device="cpu", dtype=None) -> None:
        ...
        for i in range(config.num_hidden_layers):
            ...
            if type(self.num_key_value_heads) == int:
                cur_cache_shape = global_cache_shape
                cur_sliding_cache_shape = sliding_cache_shape
            else:
                cur_cache_shape = (max_batch_size, self.num_key_value_heads[i], max_cache_len, self.head_dim)
                cur_sliding_cache_shape = ( max_batch_size, self.num_key_value_heads[i], min(config.sliding_window, max_cache_len), self.head_dim)
            cache_shape = cur_cache_shape if not self.is_sliding[i] else cur_sliding_cache_shape
            new_layer_key_cache = torch.zeros(cache_shape, dtype=self.dtype, device=device)
            new_layer_value_cache = torch.zeros(cache_shape, dtype=self.dtype, device=device)
            torch._dynamo.mark_static_address(new_layer_key_cache)
            torch._dynamo.mark_static_address(new_layer_value_cache)
