...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device, None, layer_idx, **self.rope_kwargs)
...
class LlamaMLP(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        if type(config.intermediate_size) == list:
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.hidden_size = config.hidden_size
        if type(config.num_attention_heads) == int:
        ...
        if type(config.num_key_value_heads) == int:
        ...
        self.rotary_emb = LlamaRotaryEmbedding(config=self.config, layer_idx=layer_idx)
