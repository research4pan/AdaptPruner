...
base_dir="${save_dir}/iterative_prune_train_MobileLLM_600M_instruct_1_${iterative_prune_train_step}_steps_repeat_${repeat_dataset}_strategy_${seperate_strategy}_${micro_batch_size}_microbatch_size_${lr_scheduler}_max_lr_${learning_rate}"
...
original_param_num=603188352
target_param_num=376000000
...
# model_path='/home/boyaowang/user/boyaowang/AdaptivePruning/iterative_prune_train_MobileLLM_600M_instruct_1_9_steps_repeat_1_strategy_linear_32_microbatch_size_WSD_max_lr_2e-5/trained_model/train_stage_7_of_9/checkpoint-9376'
model_path="facebook/MobileLLM-600M"
# base_training_steps=37504
for stage in $(seq 1 ${iterative_prune_train_step}); do
    ...
    python ../utils/hf_prune.py \
        ...
        --block_mlp_layer_end 40 \
        --block_attention_layer_start 0 \
        --block_attention_layer_end 40 \
    ...
    accelerate launch --config_file=fsdp_config.yaml ../utils/post_train.py \
