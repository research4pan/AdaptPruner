...
def eval_tasks_performance(model, 
                           tokenizer, 
                           tasks=["arc_challenge", "boolq", "commonsense_qa", "gsm8k","hellaswag", "MMLU", "winogrande"], 
                           num_fewshot=0, 
                           batch_size="auto"):
    ...
    for task in tasks:
        if 'acc_norm,none' in results['results'][task]:
            avg_score += results['results'][task]['acc_norm,none']
        elif 'acc,none' in results['results'][task]:
            avg_score += results['results'][task]['acc,none']
        
    avg_score /= len(tasks)

    return make_table(results), avg_score
