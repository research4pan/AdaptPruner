    ...
    def __call__(self, group, ch_groups=1, consecutive_groups=1):
        ...
        for dep, idxs in group:
            ...
            if layer.weight.grad is None:
                print(f"Warning: {layer} has no gradient computed")
                import sys
                sys.exit()
...
def cosine(layer_input, layer_output, dim, lower_is_better):
    distance = nn.functional.cosine_similarity(layer_input, layer_output, dim=dim)
    if lower_is_better:
        return -1 * distance
    else:
        return distance
    
def manhattan(layer_input, layer_output, dim, lower_is_better):
    distance = torch.sum(torch.abs(layer_input - layer_output), dim=dim)
    if lower_is_better:
        return -1 * distance
    else:
        return distance
    
def euclidean(layer_input, layer_output, dim, lower_is_better):
    distance = torch.sqrt(torch.sum((layer_input - layer_output) ** 2, dim=dim))
    if lower_is_better:
        return -1 * distance
    else:
        return distance
