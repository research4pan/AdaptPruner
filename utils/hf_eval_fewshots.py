...
def main(args):
    ...
    logger.log(f"Using seed: {args.seed}")
    ...
    for model_path in model_paths:
        ...
        try:
            ...
            try:
                ...
                logger.log(f"Error when loading tokenizer for: {model_path}")
        ...
        tasks=["truthfulqa"]
        result_table, avg_score = eval_tasks_performance(model, tokenizer, tasks=tasks, num_fewshot=1)
        logger.log("0-shot tasks performance before pruning: \n{}".format(result_table))
        logger.log("Average score across all tasks: {:.4f}".format(avg_score))

        tasks=["agieval"]
        result_table, avg_score = eval_tasks_performance(model, tokenizer, tasks=tasks, num_fewshot=0, batch_size=8)
        logger.log("0-shot tasks performance before pruning: \n{}".format(result_table))
        logger.log("Average score across all tasks: {:.4f}".format(avg_score))

        tasks=["mmlu"]
        result_table, avg_score = eval_tasks_performance(model, tokenizer, tasks=tasks, num_fewshot=5)
        logger.log("0-shot tasks performance before pruning: \n{}".format(result_table))
        logger.log("Average score across all tasks: {:.4f}".format(avg_score))
